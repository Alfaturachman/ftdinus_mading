...
<script>
    const searchResultsData = {
        "Ruang A101": "Ruang untuk perkuliahan dengan kapasitas 30 orang.",
        "Ruang B202": "Ruang seminar dengan fasilitas lengkap.",
        "Ruang C303": "Ruang laboratorium komputer dengan 20 unit PC.",
    };

    const scheduleData = {
        "Ruang A101": [{
                time: "08:00 - 10:00",
                event: "Kelas Matematika"
            },
            {
                time: "10:15 - 12:15",
                event: "Kelas Fisika"
        ...
        "Ruang B202": [{
            ...
            {
                time: "13:00 - 15:00",
                event: "Workshop Web Development"
            },
        ],
        "Ruang C303": [{
                time: "08:00 - 10:00",
                event: "Praktikum Komputer"
            },
            {
                time: "10:15 - 12:15",
                event: "Kelas Pemrograman"
            },
        ],
    };
    ...
    document.getElementById('searchInput').addEventListener('input', function() {
        ...
        if (query) {
            ...
            for (const [key, description] of Object.entries(searchResultsData)) {
                if (key.toLowerCase().includes(query.toLowerCase())) {
                    const result = `<div class="p-3 cursor-pointer" data-room="${key}">
    ...
    document.getElementById('searchResults').addEventListener('click', function(e) {
        if (e.target.closest('[data-room]')) {
            ...
            // Tampilkan jadwal ruangan yang dipilih
            if (scheduleData[room]) {
                let scheduleContent = `<h3 class="text-lg font-semibold">Jadwal ${room}</h3>`;
                scheduleData[room].forEach(schedule => {
                    scheduleContent += `<div class="p-2">
                                    <p class="p-2"><strong>${schedule.time}</strong>: ${schedule.event}</p>
                                </div>`;
                });
                scheduleResults.innerHTML = scheduleContent;
                scheduleResults.style.display = 'block';
            } else {
                scheduleResults.innerHTML = '<p class="text-gray-500">Tidak ada jadwal ditemukan.</p>';
                scheduleResults.style.display = 'block';
            }
